...
export default function CategoryPage() {
    ...
    const categoryService = async (params: {
        page: number;
        size: number;
        search?: string;
        sortBy?: string;
    ...
    const [formValues, setFormValues] = useState<Partial<CategoryResponse>>({});
    const [refreshKey, setRefreshKey] = useState(0);

    const formFields: FieldConfig<CategoryResponse>[] = [
        // {
        //     type: "text",
        //     name: "code",
        //     label: "Mã",
    ...
    const handleSubmit = async () => {
        try {
            const res = await updateCategory(formValues.id!, formValues);
            setOpen(false);
    ...
    const handleDelete = async (record: CategoryResponse) => {
            try {
    ...
    return (
        <div>
            ...
            <ModalForm
                ...
                values={formValues as CategoryResponse}
